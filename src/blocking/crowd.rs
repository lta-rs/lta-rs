use crate::blocking::{Client, LTAClient, Crowd, build_req_with_query, build_req_with_skip};
...
impl CrowdRequests<LTAClient> for Crowd {
    fn get_passenger_vol_by(client: &LTAClient, vol_type: VolType, date: Option<NaiveDate>, skip: Option<u32>) -> LTAResult<Vec<String>> {
        ...
        match fmt_date {
            Some(nd) => {
                build_req_with_query::<passenger_vol::PassengerVolRawResp, _, _, _>(client, url, |rb| {
                    rb.query(&[("Date", nd)])
                })
            }
            None => build_req_with_skip::<passenger_vol::PassengerVolRawResp, _, _>(client, url, skip),
        }
    }
}