...
| Traffic | `/RoadOpenings` | `getRoadOpenings` | Implemented in `openapi/lta.yaml`. Optional `$skip` is preserved; `unwrap-field: value` returns `Vec<RoadDetails>`; `StartDate` / `EndDate` decode into `satay_runtime::Date`; `EventID` decodes into the regex-validated `EventId` newtype (`^[A-Z]+-\d{6}-\d{4}$` wire format); `EventID` and `SvcDept` map to `event_id` / `service_dept`; dedicated golden fixture coverage (`tests/fixtures/road_openings/road_openings_0.json`, 20 entries) passes; generated Serde emits the wire-canonical field names and `YYYY-MM-DD` dates. | Accepted deviations, tested in `tests/traffic_road_openings.rs`: (1) the reference's `get_road_details` took a `RoadDetailsType` selector that dispatched one method between `/RoadOpenings` and `/RoadWorks`; the generated `getRoadOpenings` endpoint is dedicated to `/RoadOpenings` without the selector; (2) `lta-models` has no RoadOpenings dump, so the vendored fixture is a synthetic golden fixture modeling the DataMall wire shape (the sibling `road_works.json` shape). | None. |
| Traffic | `/RoadWorks` | `getRoadWorks` | Implemented in `openapi/lta.yaml`. Optional `$skip` is preserved; `unwrap-field: value` returns `Vec<RoadDetails>`; `StartDate` / `EndDate` decode into `satay_runtime::Date`; `EventID` decodes into the regex-validated `EventId` newtype (`^[A-Z]+-\d{6}-\d{4}$` wire format); `EventID` and `SvcDept` map to `event_id` / `service_dept`; vendored fixture coverage (`tests/fixtures/road_works/road_works_0.json`, 500 entries) passes; generated Serde emits the wire-canonical field names and `YYYY-MM-DD` dates. | Accepted deviation, tested in `tests/traffic_road_works.rs`: the reference's `get_road_details` took a `RoadDetailsType` selector that dispatched one method between `/RoadOpenings` and `/RoadWorks`; the generated `getRoadWorks` endpoint is dedicated to `/RoadWorks` without the selector, and `/RoadOpenings` will get its own endpoint. | None. |
