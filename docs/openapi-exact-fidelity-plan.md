...
| Bus | `/BusStops` | `getBusStops` | Implemented in `openapi/lta.yaml`. Optional `$skip` pagination and the changelog-6.8 optional `BusStopCode` filter are preserved; the filter serializes through the shared fixed-width `BusStopCode` newtype with leading zeroes intact. `unwrap-field: value` returns `Vec<BusStop>`; bus stop codes use the fixed-width `BusStopCode` newtype, and coordinates use bounded `Latitude` / `Longitude` newtypes. | The stricter identifier and coordinate types are accepted public-contract changes. | None. |
...
| Traffic | `/BicycleParkingv2` | `getBikeParking` | Implemented in `openapi/lta.yaml`. Required `Lat` / `Long` and optional `Dist` are preserved, with the schema `default: 0.5` applied when `Dist` is omitted and explicit `.dist(...)` overrides kept; `unwrap-field: value` returns `Vec<BikeParking>` with bounded `Latitude` / `Longitude` newtypes; `ShelterIndicator` decodes through the configured boolean-string mapping (`Y`/`Yes` → true; every other string → false via `unknown-as`); the 2759-entry vendored fixture at `tests/fixtures/bike_parking/bike_parking_0.json` passes. | Rack types remain losslessly represented as `RackType::Other(String)` rather than the reference's unit `Unknown` fallback, and variant names use satay's upper-camel acronym casing (`RacksMrt`, `RacksHdb`); wire serialization is identical (same accepted deviation as `StationCode`). The stricter coordinate types are accepted public-contract changes. | None. |
