...
fn bike_parking_decodes_every_vendored_fixture() {
    for (path, body) in common::json_fixtures("bike_parking") {
        ...
        let GetBikeParkingResponse::Ok(parking) = decoded else {
        ...
        for location in &parking {
            ...
            match location.rack_type {
                RackType::YellowBox => yellow_box += 1,
                RackType::YellowBoxPrivate => yellow_box_private += 1,
                RackType::RacksMrt => racks_mrt += 1,
                RackType::RacksBusStop => racks_bus_stop += 1,
                RackType::RacksUra => racks_ura += 1,
                RackType::RacksAva => racks_ava += 1,
                RackType::RacksIte => racks_ite += 1,
                RackType::RacksJtc => racks_jtc += 1,
                RackType::RacksPa => racks_pa += 1,
                RackType::RacksNParks => racks_nparks += 1,
                RackType::RacksHdb => racks_hdb += 1,
                RackType::RacksNlb => racks_nlb += 1,
                RackType::RacksNea => racks_nea += 1,
                RackType::Other(_) => other += 1,
            }
        }
        ...
        assert_eq!(
            [
                (RackType::YellowBox, yellow_box),
                (RackType::YellowBoxPrivate, yellow_box_private),
                (RackType::RacksMrt, racks_mrt),
                (RackType::RacksBusStop, racks_bus_stop),
                (RackType::RacksUra, racks_ura),
                (RackType::RacksAva, racks_ava),
                (RackType::RacksIte, racks_ite),
                (RackType::RacksJtc, racks_jtc),
                (RackType::RacksPa, racks_pa),
                (RackType::RacksNParks, racks_nparks),
                (RackType::RacksHdb, racks_hdb),
                (RackType::RacksNlb, racks_nlb),
                (RackType::RacksNea, racks_nea),
            ],
            [
...
fn bike_parking_response_projects_and_parses_wire_fields() {
    ...
    assert_eq!(parking[0].desc, "Bukit Panjang Road Block 403_YB");
    ...
    assert_eq!(parking[1].desc, "Bus Stop 28259");
...
fn bike_parking_serializes_to_canonical_wire_shape() {
    let parking = BicycleParking {
        desc: "Bukit Panjang Road Block 403_YB".into(),
    ...
    let parking = BicycleParking {
        ...
        rack_type: RackType::Other("Racks_XYZ".into()),
        rack_count: 10,
